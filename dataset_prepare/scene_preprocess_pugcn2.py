...
def extract_knn_patch(queries, pc, k, sem_label):
    """
    queries [M, C]
    pc [P, C]
    """
...
if __name__ == '__main__':
    ...
    save_path = '/media/1TB_HDD/pugcn_style_balance/'
    in_R = 5.5 # meter
    ...
    for seq, frame in zip(sequences, frames):
        ...
        sub_dir = ['subscene_xyz', 'subscene', 'label', 'label_origin', 'subscene_dense', 'label_dense', 
                    'pugcn_scene', 'pugcn_scene_dense', 'pugcn_label_origin', 'pugcn_label_dense_origin', 'pugcn_scene_xyz']
        for sub in sub_dir:
        ...
        for f_i in tqdm(frame, total=len(frame)):
            ...
            for wanted_label in ins_class: 
                ...
                for ins_i, ins_cnt_i in zip(ins_cls, ins_counts):
                    ...
                    # resample to 1024(input) and 2048(gt)
                    if len(mask_inds) > 5000 or len(mask_inds) < 20: # skip if too many or less points
                        continue
                    #idx = np.random.choice(mask_inds.shape[0], 1024, replace=False)
                    #mask_inds = mask_inds[idx]
                    #if dense_mask_inds.shape[0] < 2048: # skip if too many or less points
                    #    continue
                    #dense_idx = np.random.choice(dense_mask_inds.shape[0], 2048, replace=False)
                    ...
                    #np.savetxt(join('test_sample/', 'sparse.xyz'), new_points, fmt='%.6f')
                    #np.savetxt(join('test_sample/', 'dense.xyz'), dense_new_points, fmt='%.6f')
                    ...
                    ## PU-GCN filtering
                    ########################
                    KNN_pts, KNN_lbl = extract_knn_patch(p0, frame_points, k=1024, sem_label=frame_labels)
                    KNN_pts, KNN_lbl = KNN_pts.squeeze(0), KNN_lbl.squeeze(0)
                    KNN_dense, KNN_dense_lbl = extract_knn_patch(p0, dense_frame_points, k=2048, sem_label=dense_frame_labels)
                    KNN_dense, KNN_dense_lbl = KNN_dense.squeeze(0), KNN_dense_lbl.squeeze(0)
                    #np.savetxt(join('test_sample/', '1024.xyz'), KNN_pts.squeeze(0), fmt='%.6f')
                    #np.savetxt(join('test_sample/', '2048.xyz'), KNN_dense.squeeze(0), fmt='%.6f')
                    #print(KNN_pts.shape)
                    ...
                    if ins_cnt_i > 1 and has_ins: # save the subscene if ins_num > 1 and its points should be more than one
                        ...
                        ins_pts += new_points.shape[0]
                        seq_ins_cnt += 1

                        ############
                        ## PU-GCN
                        ############
                        'pugcn_label_origin', 'pugcn_label__dense_origin',
                        point_cloud.points = open3d.utility.Vector3dVector(KNN_pts)
                        open3d.io.write_point_cloud(join(save_path, 'pugcn_scene_xyz', seq, f'{seq}_{f_i}_{wanted_label}_{ins_i}.xyz'), point_cloud) 
                        point_cloud.points = open3d.utility.Vector3dVector(KNN_dense)
                        open3d.io.write_point_cloud(join(save_path, 'pugcn_scene_xyz', seq, f'{seq}_{f_i}_{wanted_label}_{ins_i}_dense.xyz'), point_cloud)

                        np.save(join(save_path, 'pugcn_scene', seq, f'{seq}_{f_i}_{wanted_label}_{ins_i}_scene.npy'), KNN_pts)
                        np.save(join(save_path, 'pugcn_scene_dense', seq, f'{seq}_{f_i}_{wanted_label}_{ins_i}_dense.npy'), KNN_dense)
                        np.save(join(save_path, 'pugcn_label_origin', seq, f'{seq}_{f_i}_{wanted_label}_{ins_i}_label.npy'), KNN_lbl)
                        np.save(join(save_path, 'pugcn_label_dense_origin', seq, f'{seq}_{f_i}_{wanted_label}_{ins_i}_label.npy'), KNN_dense_lbl)

                        gcn_pts += KNN_pts.shape[0]

                    #raise ValueError('Stop')

        print(f'seqence {seq} has save {seq_ins_cnt} subscene..')
        print(f'seqence {seq} has save {ins_pts} INS points..')
        print(f'seqence {seq} has save {gcn_pts} GCN points..')

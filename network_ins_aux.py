...
class Net_conpu_v7(nn.Module):
    def __init__(self, args):
        ...
        self.Final_feature = LocalTransformer(32, 2*self.emb_dims, fea_in_dim=args.emb_dims*2, nhead=4, num_layers=2, drop=0.1, prenorm=True)

        ## 3.1. The Neural Field, MLP.
        self.sr_head = Feature_MLPNet_relu(2*self.emb_dims, 2*self.emb_dims, 128)
        ...
        ## 3.2 Ins Embedding.
        self.ins_head = Feature_MLPNet_relu(2*self.emb_dims, 16)
        ######################## END PART : LAYERS #########################
    
    #def forward(self, points_sparse, sem_info, range_info):
    def forward(self, points_sparse, sem_info, is_train=True):
        '''
        Input:
        ...
        Final_fea = self.Final_feature(sparse_embedding, points_sparse, sem_info)  # thisbatchsize, self.emb_dims*2, self.args.num_point
        ######### Feature processing(point-former), Done.

        ######### The Neural Field, MLP.
        sr_fea = self.sr_head(Final_fea).permute(0, 2, 1)
        sparse_embedding = sparse_embedding + sr_fea

...
def my_collate_fn(data):
    '''
    data: tuple of (dense_pts, sparse_pts, ins_mask, sem_labels)
    '''
    # https://stackoverflow.com/questions/44233099/what-does-zipres-mean-in-python-in-the-following-code
    dense_pts, sparse_pts, ins_mask, sem_labels = zip(*data)
    dense_pts = torch.tensor(dense_pts)
    sparse_pts = torch.tensor(sparse_pts)
    ins_mask = torch.tensor(ins_mask)
    sem_labels = torch.tensor(sem_labels)
    
    return dense_pts.long(), sparse_pts.long(), ins_mask.long(), sem_labels.long()
...
def train_epoch(net, dataloader, optimizer, scheduler, args, use_wandb=False):
    ...
    for dense, sparse, target, sem_info in tqdm(dataloader, total=len(dataloader)):
        ...
        #gen_points_batch, ins_embed = net(sparse, sem_info, range_info)
...
if __name__ == '__main__':    
    ...
    parser.add_argument('--saving_path', type=str, default='result_pugcnstyle_balance', help='ckpt saving path')
    ...
    #path = '/media/1TB_HDD/pugcn_style/'
    #path = '/media/1TB_HDD/pugcn_style_256/'
    path = '/media/1TB_HDD/pugcn_style_balance/'

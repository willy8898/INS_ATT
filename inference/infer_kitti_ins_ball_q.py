        ...
        for i in range(len(patches)):
            ...
            if p.shape[1] < 20:
                ...
                if upsample_point_np.shape[0] > 200:
                    np.savetxt(join("/media/1TB_SSD/patch_result/", str(cnt)+'_sr.xyz'), upsample_point_np, fmt='%.6f')
                    np.savetxt(join("/media/1TB_SSD/patch_result/", str(cnt)+'_in.xyz'), patch, fmt='%.6f')
                    cnt += 1
                '''
...
def net_infer(net, gt_path, input_path, xyzresult_folder, lbl_path, chfloss_txt_path='./chf_d.txt'):
    ...
    if gt_path is not None:
        ...
        for xyz_file, gt_xyz_file, lbl_file in tqdm(zip(input_path, gt_path, lbl_path), total=max_cnt):
            ...
            ts = time.time()
            # model inference
            input_list, up_point_list = inference(net, frame_points, sem_labels, patch_num_point=10000, seed_num=250)
...
if __name__ == '__main__':    
    ...
    parser.add_argument('--gt_path', type=str, default='/media/1TB_SSD/Sem_Kitti/dataset/sequences/06/velodyne/',help='SR gt(64 beam) path')
    ...
    parser.add_argument('--save_path', type=str, default='/media/1TB_HDD/kitti_trainset/kitti_ins_balance_result/seq6_result/sr_pcd/',help='xyz saving path')

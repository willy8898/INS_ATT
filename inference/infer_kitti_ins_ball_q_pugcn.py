...
def inference(net, pc, lbl_info, patch_num_point=256, patch_num_ratio=3, batch=32, radius=5.5, seed_num=None, sample_points_list=None):
    ...
    no_infer_num = 0
    batchsize = 8
    total_batchsize = len(patches) // batchsize
    #print(total_batchsize)
    with torch.no_grad():
        '''
        ...
        '''
        patch = np.zeros((batchsize, 256, 3), dtype=np.float32)
        ins = np.zeros((batchsize, 256, 1), dtype=np.int32)
        for i in range(total_batchsize):
            s_idx = i * batchsize
            e_idx = (i+1) * batchsize
            for idx, j in enumerate(range(s_idx, e_idx)):
                patch[idx] = patches[j]
                ins[idx] = instances[j]
            p = torch.from_numpy(patch).cuda() # [bs, 256, 3]
            i = torch.from_numpy(ins).type(torch.FloatTensor).cuda() # [bs, 256, 1]

            # predict
            upsampled_p = net(p, i, is_train=False) # [bs, 1024, 3]
            #print(upsampled_p.shape)
            for i_up_pts in range(batchsize):
                up_pts_i = upsampled_p[i_up_pts]
                upsample_point_np = up_pts_i.squeeze(0).cpu().detach().numpy().astype(np.float32)
                #up_point_list.append(upsampled_p.squeeze(0))
                up_point_list.append(upsample_point_np)
...
def net_infer(net, gt_path, input_path, xyzresult_folder, lbl_path, chfloss_txt_path='./chf_d.txt'):
    ...
    if gt_path is not None:
        total_chf_loss = 0
        for xyz_file, gt_xyz_file, lbl_file in tqdm(zip(input_path, gt_path, lbl_path), total=max_cnt):
            ...
            input_list, up_point_list = inference(net, frame_points, sem_labels, patch_num_point=256, seed_num=2000)
        ...
        for xyz_file in tqdm(input_path, total=len(input_path)):
            ...
            # model inference
            input_list, up_point_list = inference(net, frame_points, patch_num_point=256, patch_num_ratio=3)

            # FPS to 4* point nums      
            upsample_result_th = torch.unsqueeze(torch.cat(up_point_list, 0), dim=0) # 1xNx3
            #print(upsample_result_th.shape)
            upsampled_p_fps_id = pn2_utils.furthest_point_sample(upsample_result_th.contiguous(), 4*frame_points.shape[0])
            upsample_result_fps = pn2_utils.gather_operation(upsample_result_th.permute(0, 2, 1).contiguous(), upsampled_p_fps_id)
            upsample_result_fps = upsample_result_fps.permute(0,2,1) # 1xNx3
            #print(upsample_result_fps.shape)
            ...
            out_pcd = (upsample_result_fps.squeeze(0)).cpu().detach().numpy().astype(np.float32)
            #out_pcd = (out_pcd*furthest_distance) + centroid
            np.savetxt(join(xyzresult_folder, basename(xyz_file)), out_pcd, fmt='%.6f')
...
if __name__ == '__main__':    
    ...
    parser.add_argument('--gt_path', type=str, default='/media/1TB_SSD/Sem_Kitti/dataset/sequences/08/velodyne/',help='SR gt(64 beam) path')
    ...
    parser.add_argument('--ckpt_path', type=str, default='result_pugcnstyle/best_loss_ckpt.pt',help='checkpoint path')
    parser.add_argument('--save_path', type=str, default='/media/1TB_HDD/pugcn_style_256_result/ep50/',help='xyz saving path')

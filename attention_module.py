...
class LocalTransformer(nn.Module):
    def __init__(self, dim_feature, dim_out, fea_in_dim, nhead=4, num_layers=2, drop=0.0, prenorm=True):
        ...
        self.Sem_mlp = Feature_MLPNet_relu(1, 16, hidden_dim=8)
        #self.Range_mlp = Feature_MLPNet_relu(3, 16, hidden_dim=8)
        # Position Encoding conv.
        self.pe = nn.Sequential(
            ConvModule(3, self.nc_in // 2, 1),
            ConvModule(self.nc_in // 2, self.nc_in, 1)
            )
        #self.initial_processing = Feature_MLPNet_relu(2*self.nc_in+32, self.nc_in)
        self.initial_processing = Feature_MLPNet_relu(2*self.nc_in+16, self.nc_in)
    ...
    def forward(self, Geo_fea, xyz, Sem_info):
        '''
        Input:
        ...
        Sem_fea_init = self.Sem_mlp(Sem_info.permute(0, 2, 1)) # bs, 16, N
        #Range_fea_init = self.Range_mlp(Range_info.permute(0, 2, 1)) # bs, 16, N
        Pos_En = self.pe(xyz.permute(0, 2, 1)) # bs, nc_in, N

        #Init_fea = torch.cat(
        #    (Geo_fea_init, Sem_fea_init, Range_fea_init, Pos_En), dim=1)
        Init_fea = torch.cat(
            (Geo_fea_init, Sem_fea_init, Pos_En), dim=1)

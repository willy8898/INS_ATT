...
class Multi_Vis:
    def __init__(self, input_list, point_list1, point_list2, point_list3, point_list4, point_list5, 
        ...
        self.point_list4 = point_list4
        self.point_list5 = point_list5
        ...
        self.fig2 = mlab.figure('INS_AUX', bgcolor=back_clr, size=sce_size)
        self.fig2.scene.parallel_projection = False
        self.fig3 = mlab.figure('NPT', bgcolor=back_clr, size=sce_size)
        ...
        #self.fig9 = mlab.figure('MPU', bgcolor=back_clr, size=sce_size)
        self.fig9 = mlab.figure('ANGLE_10', bgcolor=back_clr, size=sce_size)
        self.fig9.scene.parallel_projection = False
        #self.fig10 = mlab.figure('PUGCN', bgcolor=back_clr, size=sce_size)
        self.fig10 = mlab.figure('ANGLE_100', bgcolor=back_clr, size=sce_size)
        self.fig10.scene.parallel_projection = False
        
        self.fig4 = mlab.figure('GT Instance', bgcolor=back_clr, size=inst_size)
        self.fig4.scene.parallel_projection = False
        self.fig5 = mlab.figure('INS_AUX Instance', bgcolor=back_clr, size=inst_size)
        self.fig5.scene.parallel_projection = False
        self.fig6 = mlab.figure('NPT Instance', bgcolor=back_clr, size=inst_size)
        ...
        #self.fig11 = mlab.figure('MPU Instance', bgcolor=back_clr, size=inst_size)
        self.fig11 = mlab.figure('ANGLE_10 Instance', bgcolor=back_clr, size=inst_size)
        self.fig11.scene.parallel_projection = False
        #self.fig12 = mlab.figure('PUGCN Instance', bgcolor=back_clr, size=inst_size)
        self.fig12 = mlab.figure('ANGLE_100 Instance', bgcolor=back_clr, size=inst_size)
        self.fig12.scene.parallel_projection = False
    ...
    def update_scene(self):
        ...
        points4_name = self.point_list4[self._id]
        if basename(points4_name).endswith('.bin'):
            self.points4 = np.fromfile(points4_name, dtype=np.float32).reshape(-1, 4)
        elif basename(points4_name).endswith('.xyz'):
            self.points4 = np.loadtxt(points4_name)
        self.points4 = self.points4[:, :3]
        points5_name = self.point_list5[self._id]
        if basename(points5_name).endswith('.bin'):
            self.points5 = np.fromfile(points5_name, dtype=np.float32).reshape(-1, 4)
        elif basename(points5_name).endswith('.xyz'):
            self.points5 = np.loadtxt(points5_name)
        self.points5 = self.points5[:, :3]
        ...
        color_9 = np.copy(self.points4[:, 2])
        color_9[color_9 < -3] = -3
        color_9[color_9 > 1] = 1
        self.activations9 = mlab.points3d(self.points4[:, 0],
                                          self.points4[:, 1],
                                          self.points4[:, 2],
                                          #np.zeros((self.pointsin.shape[0])),
                                          color_9,
                                          ...
                                          scale_mode='none',
                                          figure=self.fig9)
        self.activations9.actor.property.render_points_as_spheres =  True
        self.activations9.actor.property.point_size = 2
        #self.activations9.module_manager.scalar_lut_manager.lut.number_of_colors = len(lut)
        #self.activations9.module_manager.scalar_lut_manager.lut.table = lut
        # New title
        text = '<--- (press g for previous)' + 50 * ' ' + '(press h for next) --->'
        mlab.title(str(self._id), color=(0, 0, 0), size=0.3, height=0.01)
        mlab.text(0.01, 0.01, text, color=(0, 0, 0), width=0.98)
        ...
        color_10 = np.copy(self.points5[:, 2])
        color_10[color_10 < -3] = -3
        color_10[color_10 > 1] = 1
        self.activations10 = mlab.points3d(self.points5[:, 0],
                                          self.points5[:, 1],
                                          self.points5[:, 2],
                                          #np.zeros((self.pointsin.shape[0])),
                                          color_10,
                                          ...
                                          scale_mode='none',
                                          figure=self.fig10)
        self.activations10.actor.property.render_points_as_spheres =  True
        self.activations10.actor.property.point_size = 2
        #self.activations10.module_manager.scalar_lut_manager.lut.number_of_colors = len(lut)
        #self.activations10.module_manager.scalar_lut_manager.lut.table = lut
        # New title
        text = '<--- (press g for previous)' + 50 * ' ' + '(press h for next) --->'
        mlab.title(str(self._id), color=(0, 0, 0), size=0.3, height=0.01)
        mlab.text(0.01, 0.01, text, color=(0, 0, 0), width=0.98)
    ...
    def update_ins(self, ins_pos):
        ...
        ins5_mask = filter_radius(self.points4, radius=radius, center=ins_pos)
        ins6_mask = filter_radius(self.points5, radius=radius, center=ins_pos)
        ...
        self.ins_pts5 = self.points4[ins5_mask]
        self.ins_pts6 = self.points5[ins6_mask]
        ...
        coor_mpu = voxelize_jit(self.ins_pts5, voxel_size, scene_range)
        unique_voxel_mpu = np.unique(coor_mpu, axis=0)
        coor_pu = voxelize_jit(self.ins_pts6, voxel_size, scene_range)
        unique_voxel_pu = np.unique(coor_pu, axis=0)
        ins_miou_i = miou(unique_voxel_ins, unique_voxel_gt)
        p_miou_i = miou(unique_voxel_p, unique_voxel_gt)
        mpu_miou_i = miou(unique_voxel_mpu, unique_voxel_gt)
        pu_miou_i = miou(unique_voxel_pu, unique_voxel_gt)
        ...
        coor_mpu2 = voxelize_jit_chf(self.ins_pts5, voxel_size, scene_range)
        unique_voxel_mpu2 = np.unique(coor_mpu2, axis=0)
        coor_pu2 = voxelize_jit_chf(self.ins_pts6, voxel_size, scene_range)
        unique_voxel_pu2 = np.unique(coor_pu2, axis=0)
        ...
        mpu_chf_i = chf_loss(unique_voxel_mpu2, unique_voxel_gt2).item()
        pu_chf_i = chf_loss(unique_voxel_pu2, unique_voxel_gt2).item()
        ...
        color_11 = np.copy(self.ins_pts5[:, 2])
        color_11[color_11 < -3] = -3
        color_11[color_11 > 1] = 1
        self.activations11 = mlab.points3d(self.ins_pts5[:, 0],
                                          self.ins_pts5[:, 1],
                                          self.ins_pts5[:, 2],
                                          #np.zeros((self.ins_pts3.shape[0])),
                                          color_11,
                                          ...
                                          scale_mode='none',
                                          figure=self.fig11)
        self.activations11.actor.property.render_points_as_spheres =  True
        self.activations11.actor.property.point_size = 3
        #self.activations6.module_manager.scalar_lut_manager.lut.number_of_colors = len(lut)
        #self.activations6.module_manager.scalar_lut_manager.lut.table = lut
        # Add title
        text = '<--- (press z for previous)' + 70 * ' ' + '(press x for next) --->'
        mlab.title(f'IoU: {mpu_miou_i:.4f} / Chf: {mpu_chf_i:.4f}', color=(0, 0, 0), size=0.3, height=0.01, figure=self.fig11)
        mlab.text(0.01, 0.01, text, color=(0, 0, 0), width=0.98, figure=self.fig11)
        mlab.orientation_axes(figure=self.fig11)

        color_12 = np.copy(self.ins_pts6[:, 2])
        color_12[color_12 < -3] = -3
        color_12[color_12 > 1] = 1
        self.activations12 = mlab.points3d(self.ins_pts6[:, 0],
                                          self.ins_pts6[:, 1],
                                          self.ins_pts6[:, 2],
                                          #np.zeros((self.ins_pts3.shape[0])),
                                          color_12,
                                          ...
                                          scale_mode='none',
                                          figure=self.fig12)
        self.activations12.actor.property.render_points_as_spheres =  True
        self.activations12.actor.property.point_size = 3
        #self.activations6.module_manager.scalar_lut_manager.lut.number_of_colors = len(lut)
        #self.activations6.module_manager.scalar_lut_manager.lut.table = lut
        # Add title
        text = '<--- (press z for previous)' + 70 * ' ' + '(press x for next) --->'
        mlab.title(f'IoU: {pu_miou_i:.4f} / Chf: {pu_chf_i:.4f}', color=(0, 0, 0), size=0.3, height=0.01, figure=self.fig12)
        mlab.text(0.01, 0.01, text, color=(0, 0, 0), width=0.98, figure=self.fig12)
        mlab.orientation_axes(figure=self.fig12)
        return
    ...
    def run(self):
        ...
        self.fig9.scene.interactor.add_observer('KeyPressEvent', self.keyboard_callback)
        self.fig10.scene.interactor.add_observer('KeyPressEvent', self.keyboard_callback)
        self.fig11.scene.interactor.add_observer('KeyPressEvent', self.keyboard_callback)
        self.fig12.scene.interactor.add_observer('KeyPressEvent', self.keyboard_callback)
        ...
        mlab.sync_camera(self.fig1, self.fig9)
        mlab.sync_camera(self.fig9, self.fig1)
        mlab.sync_camera(self.fig1, self.fig10)
        mlab.sync_camera(self.fig10, self.fig1)
        ...
        mlab.sync_camera(self.fig4, self.fig11)
        mlab.sync_camera(self.fig11, self.fig4)
        mlab.sync_camera(self.fig4, self.fig12)
        mlab.sync_camera(self.fig12, self.fig4)
...
def parse_config():
    ...
    parser.add_argument('--seq', type=int, default=6, help='Sequence used in Semkitti.')
...
if __name__ == '__main__':
    ...
    angle_path = '/media/1TB_HDD/angle_result/'
    ...
    #sr_list3 = sorted(glob(join(args.sr_path, str(args.seq).zfill(2), 'mpu','*.xyz')))
    sr_list3 = sorted(glob(join(angle_path, '06_10', 'Ins_aux_result','*.xyz'))) # angle result 2
    #sr_list4 = sorted(glob(join(args.sr_path, str(args.seq).zfill(2), 'pugcn','*.xyz')))
    sr_list4 = sorted(glob(join(angle_path, str(args.seq).zfill(2), 'Ins_aux_result','*.xyz'))) # angle result
    ...
    app = Multi_Vis(input_list, pcd_list, sr_list, sr_list2, sr_list3, sr_list4, pcd_list, lbl_list, lbl_map, 
                    args.cls, 
